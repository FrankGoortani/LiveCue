  ...
  public loadConfig(): Config {
    try {
      ...
      if (fs.existsSync(this.configPath)) {
        ...
        if (
          fileConfig.apiProvider !== "openai" &&
          fileConfig.apiProvider !== "gemini" &&
          fileConfig.apiProvider !== "anthropic"
        ) {
          fileConfig.apiProvider = "gemini"; // Default to Gemini if invalid
        }

        // Sanitize model selections to ensure only allowed models are used
        if (fileConfig.extractionModel) {
          fileConfig.extractionModel = this.sanitizeModelSelection(
            fileConfig.extractionModel,
            fileConfig.apiProvider
          );
        }
        if (fileConfig.solutionModel) {
          fileConfig.solutionModel = this.sanitizeModelSelection(
            fileConfig.solutionModel,
            fileConfig.apiProvider
          );
        }
        if (fileConfig.debuggingModel) {
          fileConfig.debuggingModel = this.sanitizeModelSelection(
            fileConfig.debuggingModel,
            fileConfig.apiProvider
          );
        }

        // Merge with priority: envConfig > fileConfig > defaultConfig
        return {
          ...this.defaultConfig,
          ...fileConfig,
          ...envConfig,
      ...
      // If no config file exists, but we have env variables, use those
  ...
  private loadFromEnvironment(): Partial<Config> {
    ...
    if (process.env.EXTRACTION_MODEL) {
      config.extractionModel = this.sanitizeModelSelection(
        ...
        config.apiProvider as "openai" | "gemini" | "anthropic"
      );
    }

    if (process.env.SOLUTION_MODEL) {
      config.solutionModel = this.sanitizeModelSelection(
        process.env.SOLUTION_MODEL,
        config.apiProvider as "openai" | "gemini" | "anthropic"
      );
    }

    if (process.env.DEBUGGING_MODEL) {
      config.debuggingModel = this.sanitizeModelSelection(
        process.env.DEBUGGING_MODEL,
        config.apiProvider as "openai" | "gemini" | "anthropic"
      );
    }
    ...
    if (process.env.LANGUAGE) {
      config.language = process.env.LANGUAGE;
    }
