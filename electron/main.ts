...
// Register the livecue protocol
if (process.platform === "darwin") {
  app.setAsDefaultProtocolClient("livecue");
} else {
  app.setAsDefaultProtocolClient("livecue", process.execPath, [
...
// Force Single Instance Lock - prevent multiple instances of the app
const gotTheLock = app.requestSingleInstanceLock();

if (!gotTheLock) {
  console.log("Application instance already running. Quitting.");
  app.quit();
} else {
  // Handle second-instance event - consolidated from redundant implementations
  app.on("second-instance", (event, commandLine) => {
    console.log("Second-instance event received:", commandLine);

    // Focus existing window or create a new one if it doesn't exist
    if (!state.mainWindow) {
      createWindow();
    } else {
...
async function initializeApp() {
  try {
    ...
    const appDataPath = path.join(app.getPath("appData"), "livecue");
...
// Standard window-closed handler
